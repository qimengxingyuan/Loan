...
export class CalculatorService {
  ...
  static generateSchedule(loan: LoanWithRelations): PaymentScheduleItem[] {
    ...
    const rateChanges = [...loan.rateChanges].sort((a, b) => 
      parseLocalDate(a.effectiveDate).getTime() - parseLocalDate(b.effectiveDate).getTime()
    );
    const prepayments = [...loan.prepayments].sort((a, b) => 
      parseLocalDate(a.paymentDate).getTime() - parseLocalDate(b.paymentDate).getTime()
    );
    ...
    while (remainingPrincipal > 0.01 && currentPeriod <= originalTotalMonths * 2) {
      ...
      let prepaymentType: PrepaymentType | null = null;
      let prepaymentDate: string | null = null;
      ...
      if (prepaymentAmount > 0 && prepaymentDate && prepaymentDate !== dateStr) {
        ...
        const daysSinceLastPayment = Math.max(0, Math.round(
          (prepaymentDateObj.getTime() - lastInterestDate.getTime()) / (1000 * 60 * 60 * 24)
        ));
          ...
          if (loan.method === 'equal_installment') {
            ...
            if (prepaymentType === 'reduce_term') {
              ...
              if (lastMonthlyPayment > 0) {
                ...
                monthlyPayment = this.calculateEqualInstallmentPayment(
                  remainingPrincipal,
                  monthlyRate,
                  remainingMonths
                );
            ...
            interest = this.calculateSegmentedInterest(
              lastInterestDate,
              currentPaymentDate,
              remainingPrincipal,
              rateChanges,
            ...
            interest = this.calculateFreeRepaymentSegmentedInterest(
              lastInterestDate,
              currentPaymentDate,
              remainingPrincipal,
              rateChanges,
  ...
  // 获取下一个月还款日
  private static getNextPaymentDate(currentDate: Date, paymentDay: number): Date {
  ...
  // 计算分段利息，支持当月跨越多个利率区间
  private static calculateSegmentedInterest(
    startDate: Date,
    endDate: Date,
    principal: number,
    ...
    // 构建完整的利率区间列表（基于贷款期限）
    const ratePeriods: { startDate: Date; endDate: Date; rate: number }[] = [];
    ...
    for (const period of ratePeriods) {
      ...
      if (intersectStart < intersectEnd) {
        ...
        const days = Math.max(0, Math.round(
          (intersectEnd.getTime() - intersectStart.getTime()) / (1000 * 60 * 60 * 24)
        ));
  ...
  private static calculateFreeRepaymentSegmentedInterest(
    startDate: Date,
    endDate: Date,
    principal: number,
    rateChanges: RateChange[],
  ...
  static getRemainingPrincipalAtDate(
    ...
    let lastAppliedItem: PaymentScheduleItem | null = null;
    for (const item of timeline) {
      if (item.paymentDate > targetDate) break;
