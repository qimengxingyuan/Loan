...
import type { ForecastResult, ApiResponse } from '../../../shared/types.ts';
...
router.get('/', (req, res) => {
  try {
    ...
    if (!date || typeof date !== 'string') {
      return res.status(400).json({ success: false, error: 'Date parameter is required' } as ApiResponse<never>);
    }
    ...
    res.json({ success: true, data: forecastResult } as ApiResponse<ForecastResult>);
  } catch (error) {
    console.error('Forecast error:', error);
    res.status(500).json({ success: false, error: 'Failed to generate forecast' } as ApiResponse<never>);
  }
});
