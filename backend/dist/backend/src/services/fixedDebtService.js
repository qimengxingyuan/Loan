...
export class FixedDebtService {
    ...
    static createFixedDebt(request) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `);
        stmt.run(id, request.name, request.amount, request.description || null, request.debtDate, now, now);
        return this.getFixedDebtById(id);
    }
    // 更新固定债务
    static updateFixedDebt(id, request) {
...
//# sourceMappingURL=fixedDebtService.js.map